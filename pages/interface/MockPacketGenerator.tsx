...
const CARD = 'rounded-lg border border-blue-100 bg-white shadow-[0_10px_28px_rgba(30,64,175,0.07)]';
const INPUT = 'w-full rounded-md border border-blue-100 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-blue-700 focus:ring-2 focus:ring-blue-100';
  ...
  return (
    <div className="min-h-[calc(100vh-7rem)] p-8">
      <div className="mx-auto max-w-[1520px] space-y-6 animate-in fade-in duration-500">
        <section className="relative overflow-hidden rounded-lg border border-blue-100 bg-white px-6 py-6 shadow-[0_10px_28px_rgba(30,64,175,0.07)]">
          <div className="relative grid gap-6 xl:grid-cols-[1.2fr_0.8fr] xl:items-center">
            <div>
              <h1 className="text-3xl font-bold tracking-normal text-blue-950">模拟报文生成</h1>
              <div className="mt-6 flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-md border border-blue-100 bg-white px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-800"
                >
                ...
                {selectedType && (
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-md border border-blue-100 bg-white px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-red-200 hover:bg-red-50 hover:text-red-700"
                  >
                    <X size={16} /> 清空选择
                  </button>
                )}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-lg border border-blue-100 bg-blue-50/45 p-5 shadow-sm">
                <div className="text-xs font-semibold uppercase tracking-normal text-slate-500">交易类型</div>
                <div className="mt-3 flex items-center justify-between gap-3">
                  <div className="text-2xl font-bold text-blue-950">{transactionTypes.length}</div>
              ...
              <div className="rounded-lg border border-blue-100 bg-blue-50/45 p-5 shadow-sm">
                <div className="text-xs font-semibold uppercase tracking-normal text-slate-500">报文状态</div>
                <div className="mt-3 flex items-center justify-between gap-3">
                  <div className="text-2xl font-bold text-blue-950">{activePacket ? 1 : 0}</div>
        ...
        <section className="grid gap-6 xl:grid-cols-[1fr_1.02fr]">
          <div className={`${CARD} p-6`}>
            ...
            <div className="mt-5 flex flex-col gap-3 lg:flex-row">
              ...
              <button
                onClick={handleRefreshTypes}
                className="inline-flex items-center justify-center gap-2 rounded-md border border-blue-100 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-800"
              >
                <RefreshCw size={16} className={typesLoading ? 'animate-spin' : ''} /> 刷新
              </button>
            </div>

            <div className="mt-5 rounded-lg border border-blue-50 bg-blue-50/35 p-3">
              {bootLoading || typesLoading ? (
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                  {Array.from({ length: 12 }).map((_, index) => (
                    <div key={index} className="h-24 animate-pulse rounded-lg bg-white" />
                  ))}
                </div>
              ) : filteredTypes.length === 0 ? (
                <div className="rounded-lg border border-dashed border-blue-200 bg-white px-6 py-16 text-center text-sm text-slate-500">
                  当前没有匹配的交易类型。
                </div>
              ) : (
                      ...
                      <button
                        ...
                        className={`rounded-lg border px-4 py-4 text-left transition ${
                          checked
                            ...
                            : 'border-transparent bg-white hover:border-blue-100 hover:shadow-sm'
                        }`}
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <div className="line-clamp-2 text-sm font-semibold leading-6 text-slate-900">{item.prdName}</div>
                            <div className={`mt-3 rounded-md px-3 py-2 text-[11px] font-medium leading-5 ${checked ? 'bg-blue-50 text-blue-800 ring-1 ring-blue-100' : 'bg-slate-100 text-slate-500'}`}>
                              <span className="break-all font-mono">{item.transCode}</span>
                            </div>
                          </div>
                          <div className={`mt-0.5 flex h-6 w-6 items-center justify-center rounded-full border ${checked ? 'border-blue-700 bg-blue-700 text-white' : 'border-slate-300 text-transparent'}`}>
          ...
          <div className={`${CARD} overflow-hidden p-6`}>
            ...
            <div className="mt-5">
              {packetLoading ? (
                <div className="rounded-lg border border-blue-100 bg-blue-50/35 p-6">
                ...
                <div className="rounded-lg border border-dashed border-blue-200 bg-blue-50/35 px-8 py-24 text-center text-sm text-slate-500">
                  请选择一项交易类型。
                </div>
              ) : !activePacket ? (
                <div className="rounded-lg border border-dashed border-blue-200 bg-blue-50/35 px-8 py-24 text-center text-sm text-slate-500">
                ...
                <article className="overflow-hidden rounded-lg border border-blue-100 bg-white shadow-sm">
                  <div className="flex flex-col gap-4 border-b border-slate-100 px-6 py-5 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2 text-xs font-semibold">
                        <span className="rounded-full bg-blue-700 px-3 py-1.5 text-white">{activePacket.transCode}</span>
                        <span className="rounded-full bg-white px-3 py-1.5 text-slate-600 ring-1 ring-blue-100">{activePacket.matchedRows > 0 ? `命中 ${activePacket.matchedRows} 条` : '结果'}</span>
                      </div>
                      <h3 className="mt-3 text-2xl font-bold tracking-normal text-slate-900">{activePacket.prdName}</h3>
                    </div>
                    <button
                      onClick={() => handleCopyPacket(activePacket)}
                      className="inline-flex items-center gap-2 rounded-md border border-blue-100 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-800"
                    >
                      <Copy size={16} /> 复制 JSON
                    </button>
                  </div>
                  <div className="p-5">
                    <div className="rounded-lg border border-blue-100 bg-white p-1 shadow-inner shadow-slate-100">
                      <pre className="max-h-[720px] overflow-auto rounded-md bg-[#f5f8fc] px-6 py-5 font-mono text-[13px] leading-6 text-slate-800">
      ...
      {settingsOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/35 backdrop-blur-sm">
          <div className="absolute inset-y-0 right-0 flex w-full justify-end">
            <div className="flex h-full w-full max-w-2xl flex-col overflow-hidden bg-[#f8fbff] shadow-lg shadow-slate-950/20">
              <div className="flex items-center justify-between border-b border-blue-100 bg-white px-6 py-5">
                <div>
                  <div className="text-xs font-semibold uppercase tracking-normal text-blue-700">配置</div>
                  <h3 className="mt-1 text-2xl font-bold text-slate-900">连接设置</h3>
                </div>
                <button
                  onClick={() => setSettingsOpen(false)}
                  className="rounded-md border border-blue-100 p-2 text-slate-500 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-800"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <label className="block">
                    <div className="mb-2 text-sm font-semibold text-slate-700">服务名</div>
                ...
                {testResult && (
                  <div className={`mt-5 rounded-lg border px-5 py-4 text-sm ${testResult.success ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-amber-200 bg-amber-50 text-amber-700'}`}>
              ...
              <div className="border-t border-blue-100 bg-white px-6 py-5">
                <div className="flex flex-wrap gap-3">
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-md border border-blue-100 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-800"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-md border border-blue-100 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:bg-blue-50"
                  >
                    <RefreshCw size={16} /> 重新读取配置
                  </button>
                  <button
                    onClick={handleSaveConfig}
                    className="inline-flex items-center gap-2 rounded-md bg-blue-700 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-800"
                  >
